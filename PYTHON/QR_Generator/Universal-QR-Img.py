...
Corresp = ["00100000","00100001","00100010","00100011","00100100","00100101","00100110","00100111","00101000",
...
"01111001","01111010","01111011","01111100","01111101","01111110"]

rptPattern = ["11101100", "00010001"]

def Encodechar(Data, Listchar, Listtra):
    final = ""
    for i in range(len(Data)):
        Index = Listchar.index(Data[i])
        final += Listtra[Index]
    final += 4*"0"
    return(final)

def FillDL(Data, rpt, DataStr, MaxLen):
    Rest = MaxLen - len(DataStr)
    for i in range(Rest):
        Data += rpt[i%2]
    return(Data)
...
def Organize_DC(Mode,DataStr,LenDataMax,rptPatern,Characters,Corresp,CharLenCount,LenBlock1,LenBlock2,Blocks1,Blocks2):
    DataLong = Mode
    Count = Fill8(BinTra(len(DataStr)),CharLenCount)
    DataLong += Count
    DataLong += Encodechar(DataStr,Characters,Corresp)
    DataLong = FillDL(DataLong, rptPatern, DataStr,LenDataMax)
    Blocks = Grouping(DataLong,LenBlock1,LenBlock2,Blocks1,Blocks2)
    return(Blocks)
    

Blocks = Organize_DC(Mode,DataStr,LenDataMax,rptPattern,Characters,Corresp,Char_Len_Count,Len_Blocks1,Len_Blocks2,G1_Blocks,G2_Blocks)
